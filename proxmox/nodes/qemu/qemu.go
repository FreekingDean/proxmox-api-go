...
type Efidisk0 struct {
	...
	// The following parameters are optional
	Efitype         *Efidisk0Efitype `url:"efitype,omitempty" json:"efitype,omitempty"`                     // Size and type of the OVMF EFI vars. '4m' is newer and recommended, and required for Secure Boot. For backwards compatibility, '2m' is used if not otherwise specified.
...
func (t Agent) EncodeValues(key string, v *url.Values) error {
	return util.EncodeString(key, v, t, `[enabled=]<1|0> [,fstrim_cloned_disks=<1|0>] [,type=<virtio|isa>]`)
}
...
func (t *CreateRequest) UnmarshalJSON(d []byte) error {
	...
	for k, v := range rest {
		...
		if strings.HasPrefix(k, "hostpci") {
			idxStr := strings.TrimPrefix(k, "hostpci")
		...
		if strings.HasPrefix(k, "ide") {
			idxStr := strings.TrimPrefix(k, "ide")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
		...
		if strings.HasPrefix(k, "ipconfig") {
			idxStr := strings.TrimPrefix(k, "ipconfig")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
		...
		if strings.HasPrefix(k, "numa") {
			idxStr := strings.TrimPrefix(k, "numa")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
		...
		if strings.HasPrefix(k, "parallel") {
			idxStr := strings.TrimPrefix(k, "parallel")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
		...
		if strings.HasPrefix(k, "sata") {
			idxStr := strings.TrimPrefix(k, "sata")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
		...
		if strings.HasPrefix(k, "scsi") {
			idxStr := strings.TrimPrefix(k, "scsi")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
		...
		if strings.HasPrefix(k, "serial") {
			idxStr := strings.TrimPrefix(k, "serial")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
		...
		if strings.HasPrefix(k, "usb") {
			idxStr := strings.TrimPrefix(k, "usb")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
		...
		if strings.HasPrefix(k, "virtio") {
			idxStr := strings.TrimPrefix(k, "virtio")
			idx, err := strconv.Atoi(idxStr)
			if err != nil {
				return err
			}
